...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public List<PaymentVO> main_mypage_Myreservationlist(String userid) throws SQLException {
		...
		try {
			...
			String sql = " SELECT * " + " FROM ( "
					...
					+ "    s.FK_SEQ_MOVIE_NO, " + "    m.POSTER_FILE, " + "    m.MOVIE_TITLE, "
					...
					+ " ON s.FK_SEQ_MOVIE_NO = m.SEQ_MOVIE_NO " + " WHERE p.FK_USER_ID = ? AND p.PAY_STATUS = '결제 완료' AND SYSDATE <= (s.START_TIME - INTERVAL '20' MINUTE) "
					+ " GROUP BY p.FK_USER_ID, p.IMP_UID, s.FK_SEQ_MOVIE_NO, m.POSTER_FILE, m.MOVIE_TITLE, p.PAY_SUCCESS_DATE, START_TIME,  p.PAY_AMOUNT, p.PAY_STATUS "
	...
	public List<MemberVO> Mylastpwdchangedate(Map<String, String> paraMap) throws SQLException {
		...
		try {
			...
			String sql = " SELECT PWD_CHANGE_DATE  " + " FROM tbl_member where USER_ID = ? ";
