...
<%
    ...
    int daysInMonth = today.lengthOfMonth(); // 현재 달의 일수
    int endDay = Math.min(startDay + 6, daysInMonth); // 최대 달의 마지막 날까지 표시
%>
...
<div class="container my-4">
    ...
    <div class="date-selector">
        ...
        <% if (endDay < daysInMonth) { %>
            <button type="button" class="pagination-button" onclick="changePage(<%= pageIndex + 1 %>)">다음 &gt;</button>
        <% } %>
    </div>

    <form id="dateForm" action="<%= ctxPath %>/movie/searchDate.mp" method="get" style="display: none;">
        <input type="hidden" id="selectedDate" name="selectedDate" value="<%= selectedDate %>">
        <input type="hidden" id="pageIndex" name="pageIndex" value="<%= pageIndex %>">
    </form>

    <!-- 상영 시간표 표시 -->
    <c:if test="${movieTimeList != null && not empty movieTimeList}">
...
<div class="showtime-row">
    <c:forEach var="movie1" items="${movieTimeList_o}">
        <c:if test="${movie1.movie_title == movie.movie_title}">
            <a href="<%= ctxPath %>/reservation/reservation.mp?seq_movie_no=${movie1.seq_movie_no}&start_time=${movie1.svo.start_time}&start_date=${movie1.start_date}"
...
<div class="showtime-row">
    <c:forEach var="movie2" items="${movieTimeList_t}">
        <c:if test="${movie2.movie_title == movie.movie_title}">
            <a href="<%= ctxPath %>/reservation/reservation.mp?seq_movie_no=${movie2.seq_movie_no}&start_time=${movie2.svo.start_time}&start_date=${movie2.start_date}"
